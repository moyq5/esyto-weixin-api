...
| type | 菜单类型 | int\(2\) | 否 | 0=click获取信息，1=view跳转链接, 2=media\_id(图文消息、图片、音频、视频） |
| replyType | 回复类型 | int\(2\) | 否 | 0=text文本，1=media\_id媒体素材，type=0时必填 |
| content | 回复内容 | string\(500\) | 是 | replyType=0时填写要回复的具体内容；replyType=1或者type=2时填写微信永久素材media\_id（见素材管理接口）；type=1时填写要跳转的url地址 |
